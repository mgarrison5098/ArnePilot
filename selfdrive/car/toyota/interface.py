...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), has_relay=False, car_fw=[]):  # pylint: disable=dangerous-default-value
    ...
    if candidate in [CAR.PRIUS, CAR.PRIUS_2019]:
    ...
    elif candidate in [CAR.AVALON, CAR.AVALON_2021, CAR.AVALONH_2021]:
      stop_and_go = False
      ret.safetyParam = 73
      ret.wheelbase = 2.82
      ...
      if spairrowtuning:
        ...
        ret.lateralTuning.init('indi')
        ret.lateralTuning.indi.innerLoopGain = 6
        ret.lateralTuning.indi.outerLoopGain = 6
        ret.lateralTuning.indi.timeConstant = 5.5
        ret.lateralTuning.indi.actuatorEffectiveness = 6.0
      else:
        ret.lateralTuning.pid.kpV, ret.lateralTuning.pid.kiV = [[0.5], [0.1]]
      ...
      #ret.longitudinalTuning.kpV = [0.4, 0.36, 0.325]  # braking tune from rav4h
      #ret.longitudinalTuning.kiV = [0.195, 0.10]
      ret.longitudinalTuning.kpV = [0.35, 0.3, 0.325]  # braking tune from rav4h
      ret.longitudinalTuning.kiV = [0.068, 0.10]
      stop_and_go = True
      ret.safetyParam = 55
